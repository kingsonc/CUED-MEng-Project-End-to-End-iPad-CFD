...
struct MetalBuffer<Element>: Resource {
    ...
    func extractPointcloud() -> [simd_float3] {
        var array: [simd_float3] = []

        for n in 0..<count {
            let point = buffer.contents().advanced(by: n * stride).bindMemory(to: simd_float3.self, capacity: 1).pointee
            let confidence = buffer.contents().advanced(by: n * stride + MemoryLayout<simd_float3>.stride * 2).bindMemory(to: Float.self, capacity: 1).pointee
